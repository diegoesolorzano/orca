...
export function installPreviewTerminalKeyHandler(args: {
  ...
  terminal.attachCustomKeyEventHandler((event) => {
    ...
    switch (action.type) {
      ...
      // Why (fork): redrawActivePane is pane-scoped too — a preview dialog has no
      // pane to redraw, and the switch has no default so it must be listed here.
      case 'redrawActivePane':
      case 'clearPaneTitle':
