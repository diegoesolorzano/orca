...
export function UpdateCard(): React.JSX.Element | null {
  ...
  const forkUpdateAvailable =
    ...
    (status.state === 'available' ||
      status.state === 'downloading' ||
      status.state === 'downloaded')
  const cardContent = forkUpdateAvailable ? (
...
function ForkUpdateNotice(): React.JSX.Element {
  return (
...
// Why (fork): its own component instead of reshaping the upstream install card
// (Update button, "ready" copy) — rewriting that in place conflicted on every
// merge. This one only notifies; there is no install path.
