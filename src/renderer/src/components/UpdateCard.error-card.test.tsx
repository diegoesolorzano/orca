...
describe('UpdateCard Windows signature failures', () => {
  ...
  it('keeps the blocked-check error collapsed while preserving retry and details', () => {
    ...
    // Why (fork): retry/update never downloads in fork builds — the in-app
    // installer is neutralized (shared/fork-build.ts).
    expect(download).toHaveBeenCalledTimes(FORK_SELF_UPDATE_DISABLED ? 0 : 1)
...
describe('UpdateCard local builds', () => {
  it('does not link local versions to GitHub release downloads', () => {
    ...
    // Why (fork): the fork's available-card is notify-only — it renders no Update
    // button at all, so there is no download to assert here.
