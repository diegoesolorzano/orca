...
function ThreadRow({
  ...
  // Why (fork): double-click the task line to rename the session's terminal tab
  // (a user-set customTitle wins in paneTitleForEntry), so a row reads as what
  // you are actually working on instead of the first prompt.
  const [isRenaming, setIsRenaming] = useState(false)
  const [renameValue, setRenameValue] = useState('')
  const renameInputRef = useRef<HTMLInputElement>(null)
  const openRename = (): void => {
    setRenameValue(thread.tab.customTitle ?? taskTitle)
    setIsRenaming(true)
  }
          ...
          <div className="flex min-w-0 items-start gap-2">
            <div className="min-w-0 flex-1 space-y-0.5">
              ...
              {isRenaming ? (
                ...
                >
                  {taskTitle !== workspaceTitle ? taskTitle : null}
                </div>
              )}
              {showStatusPreview ? (
                <CommentMarkdown
