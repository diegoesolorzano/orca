...
export function installMainWindowCloseLifecycle(args: {
  ...
  // Why (fork): the renderer cancelled a Cmd+Q confirmation. Clear the isQuitting
  // latch so a later plain window close isn't misclassified as a quit; the
  // will-prevent-unload path only fires on a beforeunload veto, not a dialog cancel.
