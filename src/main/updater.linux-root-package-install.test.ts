...
describe('updater Linux root packages', () => {
  ...
  it('keeps interactive install-on-quit and remote control for non-root packages', async () => {
    ...
    // Fork: install-on-quit is force-disabled in fork builds (see shared/fork-build.ts);
    // it would overwrite the patched app with an official release.
    expect(autoUpdaterMock.autoInstallOnAppQuit).toBe(!FORK_SELF_UPDATE_DISABLED)
    expect(updater.getRemoteServerUpdateSupport()).toEqual({
      installMode: 'interactive',
      automatic: true,
      reason: 'available'
