...
export class UpdaterSetup extends UpdaterDownloadInstall {
  ...
  setupAutoUpdater(mainWindow: BrowserWindow, opts?: UpdaterSetupOptions): void {
    ...
    // Why (fork): never stage an official release for install-on-quit — it would overwrite the
    // patched app and drop the local patches. The fork still checks + notifies; the user updates
    // via orca-fork-update (merge + rebuild).
