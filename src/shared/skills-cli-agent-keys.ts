...
export const SKILLS_CLI_AGENT_KEY_BY_TUI_AGENT = {
  ...
  droid: 'droid',
  kimi: 'kimi-code-cli',
  // Why (fork): minimax/zai are Claude Code on another backend, but each wrapper
  // points CLAUDE_CONFIG_DIR at its own dir, so the claude-code target would write
  // skills where they never read. null = universal target only.
  minimax: null,
  zai: null,
